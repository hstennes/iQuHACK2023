...
def scrape_page(query, page_count, thread_pool):
    search_results = {
      'name': query,
      'posts': [],
    }
    ...
    for _ in range(page_count):
        ...
        for post in posts:
            ...
            post_tasks.append((
                thread_pool.submit(scrape_post, comments_url),
                post_title,
                comments_url
            ))
        try:
    ...
    for task in post_tasks:
        post_content = task[0].result()
        add_post(search_results, task[1].text, task[1]['href'], task[2], post_content)
